...
export default class Output extends Component {
    ...
    render({ mobileView, leftImgContain, rightImgContain, source }, { scale, editingScale, altBackground, aliasing }) {
        ...
        return (h(Fragment, null,
            h("div", { class: `${style.output} ${altBackground ? style.altBackground : ''}` },
                h("two-up", { "legacy-clip-compat": true, class: style.twoUp, orientation: mobileView ? 'vertical' : 'horizontal', 
                    ...
                    h("pinch-zoom", { class: style.pinchZoom, onChange: this.onPinchZoomLeftChange, ref: linkRef(this, 'pinchZoomLeft') },
                        h("canvas", { class: `${style.pinchTarget} ${aliasing ? style.pixelated : ''}`, ref: linkRef(this, 'canvasLeft'), width: leftDraw && leftDraw.width, height: leftDraw && leftDraw.height, style: {
                                width: originalImage ? originalImage.width : '',
                                height: originalImage ? originalImage.height : '',
                                objectFit: leftImgContain ? 'contain' : '',
                            } })),
                    h("pinch-zoom", { class: style.pinchZoom, ref: linkRef(this, 'pinchZoomRight') },
                        h("canvas", { class: `${style.pinchTarget} ${aliasing ? style.pixelated : ''}`, ref: linkRef(this, 'canvasRight'), width: rightDraw && rightDraw.width, height: rightDraw && rightDraw.height, style: {
                                width: originalImage ? originalImage.width : '',
                                height: originalImage ? originalImage.height : '',
                                objectFit: rightImgContain ? 'contain' : '',
                            } })))),
            h("div", { class: style.controls },
                h("div", { class: style.buttonGroup },
                    h("button", { class: style.firstButton, onClick: this.zoomOut },
                        h(RemoveIcon, null)),
                    editingScale ? (h("input", { type: "number", step: "1", min: "1", max: "1000000", ref: linkRef(this, 'scaleInput'), class: style.zoom, value: Math.round(scale * 100), onInput: this.onScaleInputChanged, onBlur: this.onScaleInputBlur })) : (h("span", { class: style.zoom, tabIndex: 0, onFocus: this.onScaleValueFocus },
                        h("span", { class: style.zoomValue }, Math.round(scale * 100)),
                        "%")),
                    h("button", { class: style.lastButton, onClick: this.zoomIn },
                        h(AddIcon, null))),
                h("div", { class: style.buttonGroup },
                    h("button", { class: style.firstButton, onClick: this.onRotateClick, title: "Rotate" },
                        h(RotateIcon, null)),
                    !isSafari && (h("button", { class: style.button, onClick: this.toggleAliasing, title: "Toggle smoothing" }, aliasing ? (h(ToggleAliasingActiveIcon, null)) : (h(ToggleAliasingIcon, null)))),
                    h("button", { class: style.lastButton, onClick: this.toggleBackground, title: "Toggle background" }, altBackground ? (h(ToggleBackgroundActiveIcon, null)) : (h(ToggleBackgroundIcon, null)))))));
    }
}
